...
import infodation.kikker.service.MailService;
...
import infodation.kikker.web.rest.errors.EmailAlreadyUsedException;
import infodation.kikker.web.rest.errors.LoginAlreadyUsedException;
import infodation.kikker.web.rest.util.HeaderUtil;
import infodation.kikker.web.rest.util.PaginationUtil;
...
public class UserResource {
    ...
    /**
     * GET /users/sendmail/:login : re-send email to "login" user.
     *
     * @param login the login of the user to find
     * @return the ResponseEntity with status 200 (OK) and with body the "login" user, or with status 404 (Not Found)
     */
    @GetMapping("/users/sendmail/{login:" + Constants.LOGIN_REGEX + "}")
    @Timed
    public ResponseEntity<Void> resendEmail(@PathVariable String login) {
        log.debug("REST request to get User nd re-send email : {}", login);
        User user = userService.getUserWithAuthoritiesByLogin(login).get();
        if(user == null) {
        	ResponseEntity.ok().headers(HeaderUtil.createAlert( "Not found " + login, login)).build();
        }
        mailService.sendCreationEmail(user);
        return ResponseEntity.ok().headers(HeaderUtil.createAlert( "A email sent to " + login, login)).build();
    }
